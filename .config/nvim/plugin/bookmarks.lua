...
local function read_bookmarks_file()
	local bookmarks_file = bookmarks_dir .. string.format("/%s.json", vim.g.git_head)

	local file = io.open(bookmarks_file)
	if file ~= nil then
		local json = file:read("*a")
		vim.g.bookmarks = vim.json.decode(json)
	else
		vim.g.bookmarks = {}
	end
end
local function place_bookmark_extmarks(buf)
	...
	local extmrks = vim.tbl_keys(vim.g.bookmarks[fname] or {})
...
local function write_bookmarks_file()
	...
	if file ~= nil then
		file:write(vim.json.encode(vim.g.bookmarks))
		file:close()
	end
end
...
vim.api.nvim_create_autocmd({ "TextChanged", "InsertLeave" }, {
	...
	callback = function(args)
		...
		if vim.tbl_isempty(bkmrk_locs) then
			vim.g.bookmarks[fname] = nil
		else
			vim.g.bookmarks[fname] = bkmrk_locs
		end
...
local function toggle_bookmark()
	...
	if vim.tbl_count(extmrks) > 0 then
		for extmrk in vim.iter(extmrks) do
			...
			if vim.g.bookmarks[fname] ~= nil then
				vim.g.bookmarks[fname][table.concat({ row, col }, ",")] = nil
				if vim.tbl_isempty(vim.g.bookmarks[fname]) then
					vim.g.bookmarks[fname] = nil
				end
			end
		end
		...
		if vim.g.bookmarks[fname] == nil then
			vim.g.bookmarks[fname] = { [table.concat({ row, col }, ",")] = true }
		else
			vim.g.bookmarks[fname][table.concat({ row, col }, ",")] = true
		end
